# Planering och tidsrapport iteration 6

### Analys av iteration 5
Jag fick förhinder de första dagarna av iterationen och kom inte igång med arbetet förren ganska sent in i veckan, 
därför hann jag inte heller med så mycket som jag hade viljat göra. Det jag gjorde lyckades jag å andra sidan med. 
Nu kan man läsa en lektion och göra ett test och resultatet sparas i databasen.

### Tidsrapport iteration 5
<table>
	<tr>
		<th>Krav</th>
		<th>Uppgift</th>
		<th>Status</th>
		<th>Skattad tid</th>
		<th>Verklig tid</th>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF3-Las-en-lektion-och-gor-tillhorande-test.md">AF3</a></td>
		<td>Implementation AF3 - Skriv controller och template</td>
		<td>Klart</td>
		<td>6</td>
		<td>10</td>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF3-Las-en-lektion-och-gor-tillhorande-test.md">AF3</a></td>
		<td>Implementation AF3 - Skriv automatiska tester</td>
		<td>Klart</td>
		<td>3</td>
		<td>2</td>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF4-Visa-statistik-over-gjorda-tester.md">AF4</a></td>
		<td>Implementation AF4 - Skriv CSS</td>
		<td>Ej påbörjat</td>
		<td>2</td>
		<td></td>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF4-Visa-statistik-over-gjorda-tester.md">AF4</a></td>
		<td>Implementation AF4 - Skriv controller och template</td>
		<td>Ej påbörjat</td>
		<td>3</td>
		<td></td>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF4-Visa-statistik-over-gjorda-tester.md">AF4</a></td>
		<td>Implementation AF4 - Skriv automatiska tester</td>
		<td>Ej påbörjat</td>
		<td>1</td>
		<td></td>
	</tr>
	<tr>
		<td></td>
		<td>Publicera applikationen på webbhotellet</td>
		<td>Ej påbörjat</td>
		<td>2</td>
		<td></td>
	</tr>
	<tr>
		<td></td>
		<td>Skriv och genomför testfall, skriv testrapport</td>
		<td>Klart</td>
		<td>6</td>
		<td>1</td>
	</tr>
	<tr>
		<td></td>
		<td>Refactoring och bugfix</td>
		<td>Klart</td>
		<td>3</td>
		<td>1</td>
	</tr>
	<tr>
		<td></td>
		<td>Analys och arbete med gränssnitt</td>
		<td>Klart</td>
		<td>4</td>
		<td>4</td>
	</tr>
	<tr>
		<td></td>
		<td></td>
		<td>Summa</td>
		<td>30</td>
		<td>18</td>
	</tr>
	<tr>
		<td></td>
		<td></td>
		<td>Tid sedan föregående iteration</td>
		<td></td>
		<td>157</td>
	</tr>
	<tr>
		<td></td>
		<td></td>
		<td>Tid totalt i projektet</td>
		<td></td>
		<td>175</td>
	</tr>
</table>

### Mål iteration 6
Den här veckan ska jag göra en sida för att visa upp de sparade testresultaten, resten av iterationen kommer att 
ägnas åt design och utformning av applikationen. I slutet av veckan ska jag göra en test-session där det ingår att 
skriva testfall och att skriva en testrapport.

### Planering iteration 6
<table>
	<tr>
		<th>Krav</th>
		<th>Uppgift</th>
		<th>Status</th>
		<th>Skattad tid</th>
		<th>Verklig tid</th>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF4-Visa-statistik-over-gjorda-tester.md">AF4</a></td>
		<td>Implementation AF4 - Skriv CSS</td>
		<td>Klart</td>
		<td>2</td>
		<td>2</td>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF4-Visa-statistik-over-gjorda-tester.md">AF4</a></td>
		<td>Implementation AF4 - Skriv controller och template</td>
		<td>Klart</td>
		<td>3</td>
		<td>4</td>
	</tr>
	<tr>
		<td><a href="/../Anvandningsfall/AF4-Visa-statistik-over-gjorda-tester.md">AF4</a></td>
		<td>Implementation AF4 - Skriv automatiska tester</td>
		<td>Klart</td>
		<td>1</td>
		<td>1</td>
	</tr>
	<tr>
		<td></td>
		<td>Designa startsidan, mockup</td>
		<td>Klart</td>
		<td>2</td>
		<td>10</td>
	</tr>
	<tr>
		<td></td>
		<td>Designa startsidan, CSS</td>
		<td>Klart</td>
		<td>2</td>
		<td>5</td>
	</tr>
	<tr>
		<td></td>
		<td>Designa registreringsformuläret - CSS</td>
		<td>Klart</td>
		<td>2</td>
		<td>2</td>
	</tr>
	<tr>
		<td></td>
		<td>Designa startsidan som inloggad - CSS, Designa redigeringsformuläret - CSS</td>
		<td>Klart</td>
		<td>1</td>
		<td>1</td>
	</tr>
	<tr>
		<td></td>
		<td>Designa raderingsformuläret - CSS, Designa ändra lösenordsformuläret - CSS</td>
		<td>Klart</td>
		<td>1</td>
		<td>1</td>
	</tr>
	<tr>
		<td></td>
		<td>Designa lektion - CSS, Designa quiz - CSS</td>
		<td>Klart</td>
		<td>2</td>
		<td>2</td>
	</tr>
	<tr>
		<td></td>
		<td>Publicera applikationen på webbhotellet</td>
		<td>Klart</td>
		<td>2</td>
		<td>3</td>
	</tr>
	<tr>
		<td></td>
		<td>Skriv och genomför testfall, skriv testrapport</td>
		<td>Klart</td>
		<td>3</td>
		<td>2</td>
	</tr>
	<tr>
		<td></td>
		<td>Refactoring av CSS-filerna</td>
		<td>Klart</td>
		<td>3</td>
		<td>1</td>
	</tr>
	<tr>
		<td></td>
		<td></td>
		<td>Summa</td>
		<td>24</td>
		<td>34</td>
	</tr>
</table>